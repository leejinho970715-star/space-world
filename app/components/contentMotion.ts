...
export function animateContent(scope:HTMLElement){
  scope.querySelectorAll<HTMLElement>(headlineSelector).forEach((heading)=>{if(heading.dataset.animated)return;heading.dataset.animated="true";gsap.from(heading,{opacity:0,y:32,duration:.75,ease:"power3.out",scrollTrigger:{trigger:heading,start:"top 88%",once:true}})});
  const constellationChars=scope.querySelectorAll<HTMLElement>(".constellation-copy .type-char");
  if(constellationChars.length)gsap.from(constellationChars,{opacity:0,y:".8em",scale:.55,rotation:4,duration:.42,stagger:.025,ease:"back.out(2.4)",scrollTrigger:{trigger:".constellation-copy",start:"top 84%",once:true}});
