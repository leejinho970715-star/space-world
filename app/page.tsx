...
export default function Home() {
  ...
  return <main ref={root}>
    ...
    <section className="constellation-explorer"><div className="cinematic-sticky"><div className="constellation-copy"><p className="eyebrow"><i/>CELESTIAL NAVIGATION · 0{activeConstellation+1} / 0{constellations.length}</p><h2 aria-label="Connect the night sky."><AnimatedChars text="Connect the"/><br/><span><AnimatedChars text="night sky."/></span></h2><div className="constellation-menu">{constellations.map((name,i)=><button className={activeConstellation===i?"active":""} key={name} onClick={()=>setActiveConstellation(i)}><span>0{i+1}</span>{name}</button>)}</div><p className="constellation-fact step-change" key={constellationFacts[activeConstellation]}>{constellationFacts[activeConstellation]}</p></div><div className="constellation-stage"><img key={constellations[activeConstellation]} className={`constellation-step constellation-variant-${activeConstellation}`} src="/object-constellation-transparent.png" alt={`${constellations[activeConstellation]} constellation`} style={{transform:`rotate(${activeConstellation*34}deg) scale(${1-activeConstellation*.035})`}}/><span key={`${constellations[activeConstellation]}-name`} className="constellation-name step-change">{constellations[activeConstellation]}</span></div><img className="constellation-mascot" src="/astronaut-guide.png" alt="Mio pointing toward the constellations"/></div></section>
